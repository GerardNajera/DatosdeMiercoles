...
e<-ggplot(resultados_cmff, aes(x=as.factor(anio), y=goles)) +geom_boxplot() +labs(title="Pokemon (Primera GenraciÃ³n)",x="Tipo de pokemon (Primera generaciÃ³n)", 
                                                                                                         y = "Fuerza especial ataque")+theme_minimal()
table(resultados_cmff$anio,resultados_cmff$goles)
